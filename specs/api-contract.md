...
- `GET /storage/replay-manifest` is a sanitized read-only manifest over the replayed append-only record stream; it returns `{ "item": { "record_count": number, "record_kind_buckets": object, "canonical_record_hash": string } }`, uses the same JSONL/SQLite canonical record stream semantics, and never returns local paths, raw evidence refs, payloads, metadata dumps, tokens, webhooks, tmux/filesystem reads, append-only writes, or control-plane actions.
