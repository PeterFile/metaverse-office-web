...
- opt-in local task evidence ingestion is enabled only by `METAVERSE_OFFICE_TASK_EVIDENCE_FILE`; unset preserves empty task evidence input, the file must contain a JSON array or JSONL facts owned by the deployment/controller environment with safe `task_ref`, supported fixture `source_kind`, valid `observed_at`, and safe `correlation_id`, optional safe `agent_id` maps only to seeded agents, JSON array facts receive `source_format=json_array`, `source_index=N`, `source_input_ordinal=1`, and `source_file_ordinal=1`, JSONL facts receive `source_format=jsonl`, physical `line=N`, `source_index=N-1`, `source_input_ordinal=1`, and `source_file_ordinal=1`, and missing/unreadable input, invalid file content, unsupported source kinds, invalid facts, local paths, secret-shaped identifiers, or raw payload-only facts fail closed before any collector-snapshot append
