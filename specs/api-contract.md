...
- the first Phase 1 UI shell consumes `GET /office/overview`, `GET /office/operations`, `GET /agents/:id/workflow`, `GET /incidents`, `GET /timeline`, and `GET /correlations/:correlation_id` only
- the shell defaults to `GET /agents/:id/workflow?limit=10&window=60m` for the operator drawer slice, `GET /incidents?limit=10&window=60m` for the global incident feed, `GET /timeline?limit=10&window=60m` for replay pivots, and `GET /correlations/:correlation_id?limit=10&window=60m` when an operator opens a correlation drill-down
