...
Updated: 2026-05-20T09:47:25+08:00
...
- `GET /evidence-records/:evidence_id/provenance-bundle` is a read-only provenance bundle for one replayed `evidence_record`; it returns `404` with `error: "not_found"` when missing, returns bounded record fields plus nullable `snapshot`, `source`, and `replay` anchors when their stored source fields exist, and never exposes raw local paths, raw evidence refs, raw payloads, metadata, degraded reason arrays, secrets, tmux/filesystem reads, append-only writes, or control-plane actions
...
- opt-in Hermes runtime facts read from `METAVERSE_OFFICE_HERMES_RUNTIME_SOURCES_PATHS` or legacy `METAVERSE_OFFICE_HERMES_RUNTIME_SOURCES_FILE` persist only bounded source provenance in evidence metadata: every fact may expose abstract 1-based `source_input_ordinal` and `source_file_ordinal`, JSON arrays expose `source_format: json_array` plus zero-based `source_index`, and JSONL exposes `source_format: jsonl`, zero-based `source_index`, and physical `line`; collection error details use abstract source labels and must not leak configured local input paths, expanded file paths, temp roots, or raw input snippets; raw runtime metadata/payloads are not evidence metadata
