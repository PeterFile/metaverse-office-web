...
- `GET /storage/index-health` returns sanitized backend, canonical record counts, sidecar counts, drift counts, sidecar status, record-kind buckets, and latest safe record timestamp; JSONL reports `not_applicable` with null sidecar/drift counts, while SQLite stale or unreadable sidecars report `degraded/stale` without exposing local paths, stderr, payloads, metadata, or running writes/migrations
