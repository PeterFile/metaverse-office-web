...
const SELECTED_AGENT_DRILLDOWN_TAB_ATTRIBUTE_BY_LABEL: Record<SelectedAgentDrilldownTabLabel, string> = {
  Now: 'now',
  Evidence: 'evidence',
  'Replay / Correlation': 'replay'
};

function resolveExpectedSelectedAgentHubCategory(
  tabLabel: SelectedAgentDrilldownTabLabel,
  currentCategory: string | null
) {
  if (tabLabel === 'Replay / Correlation') {
    return 'replay';
  }

  if (tabLabel === 'Evidence') {
    return 'evidence';
  }
...
async function selectSelectedAgentDrilldownTabIfPresent(
  page: Page,
  ...
  const tab = tablist.getByRole('tab', { name: tabLabel });
  await expect(tab).toBeVisible();

  const currentCategory = await detailsPanel.getAttribute('data-active-hub-category');
  const expectedCategory = resolveExpectedSelectedAgentHubCategory(tabLabel, currentCategory);
  const tabIsSelected = (await tab.getAttribute('aria-selected')) === 'true';

  if (!tabIsSelected || currentCategory !== expectedCategory) {
    await tab.click();
  }
  ...
  await expect(detailsPanel).toHaveAttribute(
    ...
    SELECTED_AGENT_DRILLDOWN_TAB_ATTRIBUTE_BY_LABEL[tabLabel]
  );
  await expect(detailsPanel).toHaveAttribute('data-active-hub-category', expectedCategory);
}

async function revealSelectedAgentDrilldownControlIfNeeded(page: Page, locator: Locator, detailsPanel?: Locator) {
...
async function focusHubControlWithTab(
  ...
  options: {
    ...
    reverse?: boolean;
    detailsPanel?: Locator;
    selectedAgentDrilldownTab?: SelectedAgentDrilldownTabLabel;
  } = {}
) {
  const { maxTabs = 128, reverse = false, detailsPanel, selectedAgentDrilldownTab } = options;
  if (detailsPanel && selectedAgentDrilldownTab) {
    await selectSelectedAgentDrilldownTabIfPresent(page, detailsPanel, selectedAgentDrilldownTab, { required: true });
  } else {
    await revealSelectedAgentDrilldownControlIfNeeded(page, locator, detailsPanel);
  }
    ...
    .poll(async () =>
      page.evaluate(() => {
        ...
        if (!(activeElement instanceof HTMLElement) || activeElement === document.body) {
          return 'missing-active-element';
        }

        let current: HTMLElement | null = activeElement;
        while (current) {
          if (current.hidden || current.getAttribute('aria-hidden') === 'true') {
            return `hidden:${current.tagName.toLowerCase()}`;
          }

          const style = window.getComputedStyle(current);
          if (style.display === 'none' || style.visibility === 'hidden' || style.visibility === 'collapse') {
            return `css-hidden:${current.tagName.toLowerCase()}`;
          }
...
test.describe('operator shell smoke', () => {
  ...
  test('keeps landscape selected-agent world-shell safe-lane reachability on read-only GET routes once the Hub-close transition settles', async ({
    ...
    const expectedSourceMatrixSearch = '?newest_first=true&limit=200';
    ...
    try {
      ...
      expect(
        readPostCloseGetRequests().filter(
          (request) =>
            request.pathname === '/agents/evidence-spine/source-matrix' && request.search !== expectedSourceMatrixSearch
        )
      ).toEqual([]);
      expect(
  ...
  test('opens a selected-agent current-operation correlation drilldown via keyboard traversal', async ({ page }) => {
    ...
    await focusHubControlWithTab(
      ...
      { detailsPanel, selectedAgentDrilldownTab: 'Now' }
    );
  ...
  test('keeps the top-level selected-agent correlation evidence jump focused on shared memory without changing selection or active correlation via keyboard traversal', async ({
    ...
    await focusHubControlWithTab(
      ...
      {
        detailsPanel,
        selectedAgentDrilldownTab: 'Replay / Correlation'
      }
    );
    await expect(evidenceJumpButton).toBeFocused();
  ...
  test('keeps the active accountability correlation and request scope when opening an audit-signal responsibility chain pivot via keyboard traversal', async ({
    ...
    await focusHubControlWithTab(
      ...
      { detailsPanel, selectedAgentDrilldownTab: 'Evidence' }
    );
  ...
  test('keeps the selected-agent collector tmux preview jump focused on shared memory without changing selection, correlation, or request scope via keyboard traversal', async ({
    ...
    await expect(page.getByRole('region', { name: 'Hub focus ribbon' }).getByText('App Engineering Agent')).toBeVisible();
    await expect(detailsPanel).toHaveAttribute('data-active-hub-category', 'memory');
    await expect(detailsPanel).toHaveAttribute('data-selected-agent-drilldown-tab', 'evidence');
    await expect(memorySection.getByText('Request scope · app-engineering · corr-revenue-handoff')).toBeVisible();
    await expect(memorySection.getByText('Collector tmux preview')).toBeVisible();
    await expect(memorySection.getByText(`app-engineering · ${tmuxPreviewLabel}`)).toBeVisible();
    await expect(focusedSharedMemoryRecord).toHaveCount(1);
  ...
  test('keeps the active correlation when opening a correlation incident actor pivot via keyboard traversal', async ({ page }) => {
    ...
    await focusHubControlWithTab(
      ...
      { detailsPanel, selectedAgentDrilldownTab: 'Replay / Correlation' }
    );
  ...
  test('keeps the selected-agent workflow interaction evidence jump focused on shared memory without changing selection or active correlation via keyboard traversal', async ({
    ...
    await expect(detailsPanel).toHaveAttribute('data-selected-agent-drilldown-tab', 'evidence');
    await expect(detailsPanel).toHaveAttribute('data-active-hub-category', 'memory');
    await expect(memorySection.getByText('Focused exact artifact · /tmp/revenue-handoff.md')).toBeVisible();
    await expect(memorySection.getByText('Focused exact jump')).toBeVisible();
    await expect(focusedSharedMemoryRecord).toHaveCount(1);
    await expect(focusedSharedMemoryRecord).toContainText('Ref · /tmp/revenue-handoff.md');
  ...
  test('keeps selected-agent workflow details pinned when a refresh-only workflow 404 arrives before overview drops the agent', async ({ page }) => {
    ...
    await selectSelectedAgentDrilldownTabIfPresent(page, detailsPanel, 'Evidence');
    await expect(workflowSection.getByText('unknown agent app-engineering')).toBeVisible();
    await expect(page.getByRole('region', { name: 'Hub focus ribbon' }).getByText('App Engineering Agent')).toBeVisible();
    await expect(page.getByRole('button', { name: 'Clear Selection' })).toBeVisible();
  });
