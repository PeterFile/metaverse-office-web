...
- task evidence projection is a local normalization and opt-in file ingestion contract: allowlisted fixture facts with `source_kind` `kanban_fixture`, `linear_fixture`, `slack_fixture`, or `task_fixture` may be projected to canonical `evidence_record` payloads with `evidence_ref: task://<source_kind>/<task_ref>`, `evidence_role: task_reference`, `output_candidate: false`, safe `task_ref`/optional `fact_id` metadata, optional abstract input proof metadata (`source_format`, `source_index`, JSONL `line`, `source_input_ordinal`, `source_file_ordinal`), and no raw title/comment/body/description/path/secret/payload fields; invalid, unsupported, or secret-shaped identifiers are rejected or degraded without leaking raw values
...
- opt-in local task evidence ingestion is enabled by `METAVERSE_OFFICE_TASK_EVIDENCE_PATHS` or legacy `METAVERSE_OFFICE_TASK_EVIDENCE_FILE`; unset preserves empty task evidence input, `PATHS` takes precedence over `FILE`, `PATHS` is `path.delimiter`-separated and may name files or directories, directories expand only local `.json`/`.jsonl` files in stable lexical order, inputs must contain JSON arrays or JSONL facts owned by the deployment/controller environment with safe `task_ref`, supported fixture `source_kind`, valid `observed_at`, and safe `correlation_id`, optional safe `agent_id` maps only to seeded agents, JSON array facts receive `source_format=json_array`, `source_index=N`, and abstract `source_input_ordinal`/`source_file_ordinal`, JSONL facts receive `source_format=jsonl`, physical `line=N`, `source_index=N-1`, and abstract `source_input_ordinal`/`source_file_ordinal`, and missing/unreadable input, invalid file content, unsupported source kinds, invalid facts, local paths, secret-shaped identifiers, or raw payload-only facts fail closed before any collector-snapshot append
