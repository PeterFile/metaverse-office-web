...
- `GET /evidence-records` is read-only evidence inspection over replayed internal evidence records; it filters exact `agent_id`, `source_kind`, `evidence_role`, `output_candidate`, optional `newest_first=true`, and `limit`, and must not collect, read tmux/filesystems, append records, or expose control-plane actions.
