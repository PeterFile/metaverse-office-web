...
async function focusHubControlWithTab(page: Page, locator: Locator, accessibleName: string, maxTabs = 32) {
...
test.describe('AI Town shell smoke', () => {
  ...
  test('opens selected-agent detail from a crew-overview incident agent pivot via keyboard traversal', async ({ page }) => {
    await page.goto('/');
    await page.getByRole('button', { name: 'Open Hub' }).click();
