...
- `GET /collectors/controller-snapshot/schema` is a local-only read-only static contract catalog for collector snapshot clients/tests; it returns `{ "item": { "routes": array, "source_kinds": string[], "source_statuses": string[], "confidence_levels": string[], "snapshot_fields": object, "limit": { "default": 50, "max": 200 }, "route_write_boundary": string } }`, catalogs only the latest, summary, evidence-coverage, source-health, history, and diff routes with bounded filter names, exact public `source_kind` filter values/aliases, and sanitized top-level field names, ignores query filters, and never inspects replayed snapshots, collects, reads tmux/filesystems/runtime sources, appends records, or returns actual ids, refs, paths, degraded reasons, payloads, metadata, secrets, liveness/productivity/severity claims, or control-plane actions
