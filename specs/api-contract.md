...
- opt-in local task evidence ingestion is enabled by `METAVERSE_OFFICE_TASK_EVIDENCE_PATHS` or legacy `METAVERSE_OFFICE_TASK_EVIDENCE_FILE`; unset preserves empty task evidence input, `PATHS` takes precedence over `FILE`, `PATHS` is `path.delimiter`-separated and may name files or directories, directories expand only local `.json`/`.jsonl` files in stable lexical order, inputs must contain JSON arrays or JSONL facts owned by the deployment/controller environment with safe `task_ref`, supported fixture `source_kind`, valid `observed_at`, and safe `correlation_id`, optional safe `agent_id` maps only to seeded agents, JSON array facts receive `source_format=json_array`, `source_index=N`, and abstract `source_input_ordinal`/`source_file_ordinal`, JSONL facts receive `source_format=jsonl`, physical `line=N`, `source_index=N-1`, and abstract `source_input_ordinal`/`source_file_ordinal`, and missing/unreadable input, invalid file content, unsupported source kinds, invalid facts, local paths, secret-shaped identifiers, top-level/nested control-field categories, or raw payload-only facts fail closed before any collector-snapshot append
