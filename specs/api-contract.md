...
- opt-in Hermes runtime facts read from `METAVERSE_OFFICE_HERMES_RUNTIME_SOURCES_PATHS` or legacy `METAVERSE_OFFICE_HERMES_RUNTIME_SOURCES_FILE` persist only bounded source provenance in evidence metadata: JSON arrays expose `source_format: json_array` plus `source_index`, and JSONL exposes `source_format: jsonl`, `source_index`, and physical `line`; collection error details use abstract source labels and must not leak configured local input paths, expanded file paths, temp roots, or raw input snippets; raw runtime metadata/payloads are not evidence metadata
