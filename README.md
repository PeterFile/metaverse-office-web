...
- evidence records can be deep-linked through `GET /evidence-records/:evidence_id`; it returns `{ "item": evidence_record }` using the same record shape as list items, returns `404` with `error: "not_found"` for unknown ids without echoing the requested id, and does not shadow `GET /evidence-records/summary`
