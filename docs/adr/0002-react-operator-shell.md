...
## Context
The user explicitly approved React for the frontend while keeping the evidence-first rule intact: UI must not outrun the event/state/storage/query architecture. The backend exposes read-only office overview, operations, agent workflow, incident feed, timeline, collector, runtime source-gap, replay, memory, peer-watch, and correlation routes that power the operator shell without requiring UI-owned write paths.
...
- drive the operator shell from the current read-only route contracts listed in `specs/api-contract.md` and summarized in `README.md`; this ADR records the React/Vite/read-only decision and intentionally does not maintain an exhaustive living route catalog. Current examples include `GET /office/overview`, `GET /office/operations`, `GET /agents/:id/workflow`, collector evidence/source-health reads, runtime source-gap reads, evidence-record/detail/provenance/context/replay reads, accountability replay/checkpoint reads, memory, peer-watch, and correlation drill-down reads.
