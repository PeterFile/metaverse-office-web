...
- `GET /runtime/source-gaps/reason-summary` is the compact read-only reason-code bucket summary over the same safe filters as `GET /runtime/source-gaps` excluding append-index cursors; it counts only non-task degraded/missing/error source records, returns `{ "item": { "total_count": number, "returned_limit": number, "reason_code_buckets": object, "source_kind_buckets": object, "source_status_buckets": object } }`, uses fixed bounded reason-code keys derived from internal degraded/missing/error reasons, applies counts before `limit`, and never returns raw degraded reasons, evidence ids, refs, metadata, paths, tmux/Hermes/session/profile refs, tokens, payloads, liveness/productivity/severity inference, collection, writes, or control-plane actions
