...
- `GET /runtime/source-gaps/schema` is a local-only read-only static contract catalog for runtime source-gap clients/tests; it returns `{ "item": { "source_kinds": string[], "evidence_roles": string[], "source_statuses": string[], "source_gap_statuses": string[], "supported_filters": string[], "boolean_filters": string[], "lifecycle_states": string[], "mapping_decisions, trend_buckets": string[], "limit": { "default": 50, "max": 200 }, "route_write_boundary": string } }`, exposes only bounded enum/filter/mapping-decision strings and limit metadata, excludes raw `evidence_ref` and append-index cursors from supported filters, ignores query filters, must be routed before source-gap row/group/trend paths, and never inspects replayed rows, collects, reads tmux/filesystems/runtime inputs, appends records, or returns actual ids, refs, paths, payloads, metadata, secrets, liveness/productivity/severity claims, or control-plane actions
...
- `GET /runtime/source-gaps/lifecycle` is a compact read-only lifecycle grouping surface over filtered evidence records; it accepts the same filters except raw `evidence_ref` and append-index cursors plus exact additive `lifecycle_state=opened|continuing|resolved|observed_unmapped`, returns `{ "item": { "total_count": number, "total_groups": number, "returned_limit": number, "groups": [{ "agent_id": string|null, "source_kind": string|null, "evidence_role": string|null, "record_count": number, "current_status": string|null, "lifecycle_state": "opened"|"continuing"|"resolved"|"observed_unmapped", "first_observed_at": string|null, "last_observed_at": string|null, "first_collected_at": string|null, "last_collected_at": string|null, "snapshot_count": number, "source_status_buckets": object }] } }`, reports `total_count` as the matching record aggregate after lifecycle-state filtering, `total_groups` as the pre-limit filtered group aggregate, and `record_count` as the safe per-group record aggregate, groups by exact `agent_id`, `source_kind`, and `evidence_role`, returns only groups that have a source-gap signal, computes and filters groups before `limit`, returns zero counts and empty groups for empty matches or unknown lifecycle states without echoing raw input, and never returns collector snapshot ids, evidence ids, refs, metadata, degraded reason arrays, paths, tmux refs, Hermes payloads, incident/severity/liveness/productivity inference, or control-plane actions
- `GET /runtime/source-gaps/transition-summary` is a compact read-only count summary over the same lifecycle groups as `GET /runtime/source-gaps/lifecycle`; it accepts the same filters except raw `evidence_ref` and append-index cursors plus exact additive `lifecycle_state=opened|continuing|resolved|observed_unmapped`, returns `{ "item": { "total_records": number, "total_groups": number, "returned_limit": number, "lifecycle_state_buckets": { "<state>": { "group_count": number, "record_count": number } }, "source_kind_buckets": object, "source_status_buckets": object, "mapped_state_buckets": object, "latest_observed_at": string|null, "latest_collected_at": string|null } }`, reports `total_records` as the matching lifecycle record aggregate after lifecycle-state filtering and `total_groups` as the matching lifecycle group aggregate before `limit`, uses `group_count` for groups in each bucket and `record_count` for member records in each bucket, keeps `source_kind_buckets` to runtime source kinds only, treats `returned_limit` as parsed route-limit metadata because no rows are returned, returns zero stable buckets and null timestamps for empty matches or unknown lifecycle states without echoing raw input, and never returns group rows, agent ids, evidence ids, refs, collector snapshot ids, metadata, degraded reason arrays, paths, tmux refs, Hermes payloads, incident/severity/liveness/productivity inference, task semantics, or control-plane actions
