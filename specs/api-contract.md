...
- `GET /storage/schema` is a local-only read-only static contract catalog for storage and replay clients/tests; it returns `{ "item": { "routes": array, "route_fields": object, "limit": { "default": 50, "max": 200 }, "route_write_boundary": string } }`, catalogs only `/storage/replay-manifest`, `/storage/index-health`, `/accountability/replay/checkpoint-summary`, and `/accountability/replay/checkpoint-log` with bounded route names, filter names, and top-level field names, ignores query filters, must be routed before future storage dynamic paths, and never inspects replayed records, runs storage probes, collects runtime sources, appends records, or returns actual ids, refs, local paths, raw sqlite paths, SQL text, payloads, metadata, degraded reasons, secrets, tokens, webhooks, runtime reads, writes, migrations, backfills, or control-plane actions.
...
- `GET /storage/index-health` is a sanitized read-only health view for append-only storage indexes; it returns `{ "item": { "backend": "jsonl"|"sqlite", "status": "ok"|"degraded", "record_count": number, "record_index_count": number|null, "record_evidence_ref_count": number|null, "record_index_drift_count": number|null, "record_evidence_ref_drift_count": number|null, "evidence_query_probe_count": number|null, "evidence_query_probe_drift_count": number|null, "evidence_query_probe_status": "complete"|"stale"|"not_applicable", "sidecar_status": "complete"|"stale"|"not_applicable", "record_kind_buckets": object, "latest_record_ts": string|null } }`. JSONL always reports `sidecar_status: "not_applicable"` and `evidence_query_probe_status: "not_applicable"` with null sidecar/probe counts and drift counts. SQLite uses read-only sidecar count/drift checks plus read-only evidence query dimension parity probes over the sidecars; missing/unreadable/stale/probe-drifting sidecars report `status: "degraded"` and stale sidecar/probe status without falling back to JSONL. The route never returns evidence ids, collector/correlation ids, local paths, raw sqlite paths, stderr, raw evidence refs, probe filter values, payloads, metadata dumps, degraded reasons, tokens, webhooks, tmux/filesystem reads, append-only writes, migrations, backfills, or control-plane actions.
