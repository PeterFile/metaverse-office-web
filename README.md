...
pnpm verify:quick -- --lane=<docs|backend|web-api|ui|ui-source-gap|smoke>
...
`pnpm verify:quick -- --changed` routes staged, unstaged, and untracked files to the narrowest safe quick lane. `pnpm verify:quick -- --since=<ref>` routes files changed since a ref. The classifier is conservative: docs-only changes use `docs`, backend-only changes use `backend`, known web API/client changes use `web-api`, source-gap/source-health UI source+test changes use `ui-source-gap`, pure Live Evidence helper/model source files with existing exact Vitest counterparts use `focused-files`, changed web Vitest files and allowlisted web helper source changes with adjacent tests use `focused-files`, and smoke/e2e changes use `smoke`. Component/container UI source changes and unknown or cross-layer changes fail closed or route to the broad `ui` lane instead of pretending to replace full CI.
...
- `GET /storage/schema` returns a static storage-route catalog with route names, top-level response field names, backend/status enum metadata, bounded path-free health reason code metadata, and the read/write boundary; it does not inspect replayed rows, sqlite files, runtime inputs, ids, refs, paths, hash values, payloads, secrets, or run writes/migrations
...
- `GET /runtime/input-inventory` returns `{ "item": { "hermes_runtime_sources": { "enabled": boolean, "mode": "unset"|"file"|"paths", "configured_input_count": number }, "task_evidence_sources": { "enabled": boolean, "mode": "unset"|"file"|"paths", "configured_input_count": number } } }`; it is derived only from startup config already parsed by the backend, honors `PATHS` over legacy `FILE`, does not stat/read/expand configured inputs, collect, append records, or expose env values, configured/expanded paths, filenames, payloads, raw refs, liveness/productivity, task dispatch, profile routing, or worker orchestration
- `GET /runtime/input-evidence-watermark` returns `{ "item": { "input_evidence_count": number, "source_family_buckets": { "hermes_runtime_sources": number, "task_evidence_sources": number }, "source_kind_buckets": object, "latest_collected_at": string|null, "latest_observed_at": string|null, "max_source_input_ordinal": number|null, "max_source_file_ordinal": number|null, "source_input_ordinal_buckets": object, "source_file_ordinal_buckets": object, "source_format_buckets": { "json_array": number, "jsonl": number } } }`; it ignores query strings and derives only from replayed `evidence_record.metadata.source_provenance` for Hermes runtime and task evidence input families, bounds ordinal bucket keys to 16 while keeping max ordinals as the freshness waterline, without reading env/files/directories, collecting, appending, or exposing configured paths, filenames, raw refs, payloads, metadata dumps, degraded reasons, tokens, task dispatch, profile routing, or worker orchestration
- `GET /runtime/agent-census/schema` returns a static read-only contract catalog for the future runtime agent census, with no filters, response field names, count-source semantics (`runtime_discovered`, `seed_roster_fallback`), agent states (`discovered`, `fallback`, `degraded`, `unknown`), safe identity field names, fallback/degraded semantics, and a read/write boundary; it does not inspect replayed rows, read runtime inputs, collect, append, infer liveness/productivity, or expose actual ids, refs, paths, payloads, metadata, tokens, task actions, profile routing, or worker orchestration
...
- `GET /collectors/controller-snapshot/schema` is a static read-only catalog for the latest, summary, append-proof, evidence-coverage, source-health, history, and diff routes; it returns only bounded route/filter/enum/limit metadata, exact public `source_kind` filter values/aliases, and static top-level field names, and must not inspect snapshots, collect, read tmux/filesystems/runtime sources, append records, or expose raw ids, refs, paths, degraded reasons, payloads, metadata, or control-plane actions
...
- `GET /collectors/controller-snapshot/append-proof` is the safe numeric append-order proof boundary; by default it returns latest, with `collector_snapshot_id` it exact-matches that replayed snapshot, unknown ids return a missing-snapshot proof without echoing input, and it returns only `snapshot_append_index`, evidence append-index count/range, `ordering_status`, and stable source/role/status/output/mapped buckets without raw refs, paths, payloads, metadata, degraded reasons, or control-plane actions
...
- `GET /collectors/controller-snapshot/diff` is a compact read-only latest-vs-previous snapshot delta, or explicit `from_collector_snapshot_id`/`to_collector_snapshot_id` (`from`/`to` aliases); it returns only snapshot ids/timestamps, summary deltas, source-health bucket deltas, count-only `runtime_source_evidence_delta`, and bounded agent-level change flags/status transitions, without raw items, paths, tmux/Hermes/session/profile refs, task refs, evidence refs, metadata, payloads, degraded reasons, task dispatch, writable Kanban, routing, orchestration, liveness/productivity, or severity claims
...
- collector evidence records capture workspace roots, workspace files, mapped tmux panes, mapped Hermes profile/session presence, and unmapped tmux/Hermes/runtime task evidence with source kind, evidence role, source status, safe `mapping_decision`, output-candidate classification, collector correlation, and degraded reasons
- task evidence records are projected only from allowlisted fixture facts (`kanban_fixture`, `linear_fixture`, `slack_fixture`, `task_fixture`) into the same canonical `evidence_record` shape, using `task://<source_kind>/<task_ref>` refs, `evidence_role: task_reference`, and `output_candidate: false`; opt-in `METAVERSE_OFFICE_TASK_EVIDENCE_PATHS` overrides legacy `METAVERSE_OFFICE_TASK_EVIDENCE_FILE`, may name files or directories, and expands directory `.json`/`.jsonl` files in lexical order with abstract ordinal/format/line provenance only; facts with claim/complete/assign/dispatch/route/writeback/mutate control fields at any object/array depth are rejected, and this does not add live task ingestion, writeback, task dispatch, profile routing, Kanban controls, or raw payload/path/secret exposure
- evidence records can be queried through `GET /evidence-records`; it returns `{ "items": [...] }`, supports exact `evidence_id`, `agent_id`, `source_kind`, `evidence_role`, `output_candidate`, `evidence_ref`, `source_status`, `collector_snapshot_id`, `correlation_id`, optional `mapped=true|false`, inclusive valid-ISO `observed_since`/`observed_until` and `collected_since`/`collected_until` windows, exact positive integer append-order cursors `after_append_index`/`before_append_index`, optional `newest_first=true`, and post-filter `limit`, includes additive numeric `append_index` anchors from canonical append order and only safe `mapping_decision` enum codes (`mapped`, `unmapped_unknown`, `non_seeded_agent`, `duplicate_source`, `shared_ref`, `unsafe_identifier`, `missing_expected_source`, `read_error`, `unknown`), ignores blank filters, invalid `mapped`, invalid timestamp filters, and invalid cursor values such as zero, negative, decimal, or suffixed strings, returns an empty `items` array for unknown exact values or contradictory filters such as `mapped=false&agent_id=app-engineering`, applies valid cursors before `newest_first` and `limit` with exclusive bounds, orders `newest_first=true` by `observed_at` descending, then `collected_at` descending, then a deterministic persisted-field tie key, and uses the existing default/max limit behavior
- evidence-record clients can read static enum/filter metadata from `GET /evidence-records/schema`; it returns bounded source-kind, evidence-role, source-status, mapping-decision, filter, boolean-filter, and limit metadata without inspecting replayed evidence rows, triggering collection, appending records, or exposing actual ids, refs, paths, payloads, metadata, secrets, runtime reads, or control-plane actions.
- agents evidence-spine clients can read static contract data from `GET /agents/evidence-spine/schema`; it returns bounded source-kind, evidence-role, source-status, filter, boolean-filter, surface, response-field, count-semantics, and limit data without inspecting replayed rows, triggering collection, appending records, reading runtime inputs, or exposing actual ids, refs, paths, payloads, metadata, secrets, liveness/productivity/severity claims, or control-plane actions.
- evidence records can be deep-linked through `GET /evidence-records/:evidence_id`; it returns `{ "item": evidence_record }` using the same record shape as list items, including the same additive numeric `append_index` anchor used by replay-window center and matching checkpoint-log rows, returns `404` with `error: "not_found"` for unknown ids without echoing the requested id, and does not shadow `GET /evidence-records/summary`
- evidence record provenance can be inspected through `GET /evidence-records/:evidence_id/provenance-bundle`; unknown ids return `404` with `error: "not_found"` without echoing the requested id; successful responses return bounded record fields plus `source_summary` (`kind`, `status`, `role`, `output_candidate`, `mapped`, and observed/collected timestamps; unknown or unsafe source enum values and invalid/unsafe timestamps are `null` rather than echoed), nullable snapshot/source/replay anchors, and optional sanitized `input_proof` with only `source_format`, `source_index`, `line`, `source_input_ordinal`, and `source_file_ordinal`, but must not expose raw evidence refs, `source_provenance`, snapshot ids or correlation ids inside `source_summary`, metadata, degraded reasons, local paths, tmux/Hermes/session/profile refs, tokens, webhooks, callback endpoints, payload text, or control-plane actions
- evidence source context can be inspected through `GET /evidence-records/:evidence_id/source-context`; it returns one safe enum-only `disclosure`, bounded `source_summary`, bounded record fields, sanitized same-evidence source-gap context, and sanitized mapped-agent source-health context when available. Unknown ids return `404` without echoing the requested id and expose only a safe deny reason. Unmapped runtime evidence stays unmapped (`source_health.agent_items: []`) and is not projected onto seeded agents. The response does not expose raw evidence refs, collector/correlation ids, local paths, tmux/Hermes/session/profile refs, metadata, degraded reasons, payloads, URLs, tokens, webhooks, runtime reads, or write/control-plane actions.
- evidence replay windows can be inspected through `GET /evidence-records/:evidence_id/replay-window?before=&after=`; it returns the selected evidence safe `source_summary` plus append-order checkpoint rows before and after it. `before` and `after` default to `2`, accept `0`, and cap at `10`; unknown ids return `404` without echoing the requested id. The response does not expose raw evidence refs, collector/correlation ids, local paths, tmux/Hermes/session/profile refs, metadata, degraded reasons, payloads, URLs, tokens, webhooks, runtime reads, writes, or control-plane actions.
- evidence records can expose safe facets through `GET /evidence-records/facets` with the same filters; it returns only `total_count`, `returned_limit`, and stable count buckets for `source_kind`, `evidence_role`, `source_status`, `mapping_decision`, `output_candidate`, mapped/unmapped, and seeded agent ids plus `unmapped`; it computes buckets before `limit`, ignores unknown or unsafe historical bucket values instead of echoing them as response keys, and does not return raw evidence rows, ids, refs, metadata, degraded arrays, paths, tmux refs, payloads, or trigger collection, filesystem/tmux reads, writes, or control-plane actions
- `GET /storage/replay-manifest` includes additive `runtime_gap_summary` derived only from replayed persisted evidence records with the same inclusion semantics as `GET /runtime/source-gaps`; it returns total/mapped/unmapped counts, bounded source-kind/source-status buckets, and latest observed/collected timestamps only, and never exposes raw evidence rows, ids, refs, collector/correlation ids, paths, metadata, degraded reasons, payloads, tokens, runtime reads, writes, liveness/productivity/severity inference, or control-plane actions
...
- evidence input proof can be summarized through `GET /evidence-records/input-proof-summary` with the same filters; it returns only `{ "item": { "total_count": number, "returned_limit": number, "proof_count": number, "missing_proof_count": number, "source_format_buckets": { "json_array": number, "jsonl": number }, "source_index_buckets": object, "line_buckets": object, "source_input_ordinal_buckets": object, "source_file_ordinal_buckets": object } }`, bounds ordinal bucket keys by the parsed `limit`, returns zero counts and empty ordinal buckets for unknown or empty matches, and does not return raw evidence ids, refs, paths, source provenance, payloads, metadata, degraded arrays, tokens, webhooks, or trigger collection, runtime reads, writes, or control-plane actions
- evidence projection safety can be audited through `GET /evidence-records/projection-audit` with the same filters except raw `evidence_ref`; it returns only `{ "item": { "total_count": number, "returned_limit": number, "input_proof_count": number, "missing_input_proof_count": number, "invalid_input_proof_count": number, "unknown_source_kind_count": number, "unknown_evidence_role_count": number, "unknown_source_status_count": number, "invalid_observed_at_count": number, "invalid_collected_at_count": number } }`, computes every counter after filters and before `limit`, exposes the parsed `returned_limit`, counts persisted-but-unprojectable input proof as `invalid_input_proof_count` without echoing it, ignores raw `evidence_ref` query input, is routed before `GET /evidence-records/:evidence_id`, and does not return raw invalid enum values, timestamps, input proof, evidence ids, refs, local paths, metadata, degraded reasons, payloads, tokens, webhooks, tmux/Hermes/session/profile refs, or trigger collection, runtime reads, writes, or control-plane actions
- evidence refs can be grouped through `GET /evidence-records/ref-rollup` with the same filters; raw `evidence_ref` remains an exact-match input filter, but response groups expose only `evidence_ref: null`, a response-local `evidence_ref_key`, a coarse `evidence_ref_label`, counts, seeded agent id buckets, `unmapped` for null or literal unmapped agent ids, `unknown` for other non-seeded agent ids, and allowlisted positive source kind/status buckets. The route groups by stored refs internally before `limit` but does not return raw refs, unallowlisted bucket keys, local paths, tmux/Hermes/session/profile refs, URLs, tokens, payloads, metadata, or a reverse lookup from safe keys to raw refs.
- `GET /runtime/source-gaps` is a compact read-only feed over replayed evidence records; it accepts the same exact/time/mapped filters except raw `evidence_ref` and append-index cursors, returns only missing/degraded/error source rows plus observed unmapped runtime evidence, omits evidence ids, raw evidence refs, metadata, and degraded reason arrays, and does not trigger collection, tmux/filesystem reads, append-only writes, incident/severity/liveness escalation, or control-plane actions
- runtime source-gap clients can read static enum/filter metadata from `GET /runtime/source-gaps/schema`; it returns bounded source-kind, evidence-role, source-status, source-gap-status, mapping-decision, reason-code, lifecycle-state, trend-bucket, filter, boolean-filter, and limit metadata without inspecting replayed rows, triggering collection, appending records, reading runtime inputs, or exposing actual ids, refs, paths, payloads, metadata, secrets, liveness/productivity/severity claims, or control-plane actions.
- `GET /runtime/source-gaps/summary` is the compact read-only bucket summary over the same filtered source-gap set; it accepts the same filters as `GET /runtime/source-gaps` excluding append-index cursors, returns `{ "item": { "total_count": number, "returned_limit": number, "mapped_count": number, "unmapped_count": number, "output_candidate_buckets": { "true": number, "false": number }, "source_kind_buckets": object, "evidence_role_buckets": object, "source_status_buckets": object, "mapping_decision_buckets": object, "collector_snapshot_id_buckets": object, "first_observed_at": string|null, "last_observed_at": string|null, "first_collected_at": string|null, "last_collected_at": string|null } }`, computes counts and lifecycle extrema before `limit`, collapses unsafe or unknown summary enum/id bucket keys into `unknown` instead of echoing them, and never returns evidence ids, refs, metadata, degraded reason arrays, paths, tmux refs, or Hermes payloads
- `GET /runtime/source-gaps/reason-summary` is the compact read-only reason-code bucket summary over the same safe filters as `GET /runtime/source-gaps` excluding append-index cursors; it counts only non-task degraded/missing/error source records, returns `{ "item": { "total_count": number, "returned_limit": number, "reason_code_buckets": object, "source_kind_buckets": object, "source_status_buckets": object, "mapped_state_buckets": { "mapped": number, "unmapped": number }, "mapping_decision_buckets": object } }`, uses fixed bounded reason-code and mapping-decision keys derived from internal degraded/missing/error reasons and safe mapping decisions, applies counts before `limit`, and never returns raw degraded reasons, evidence ids, refs, metadata, paths, tmux/Hermes/session/profile refs, tokens, payloads, liveness/productivity/severity inference, collection, writes, or control-plane actions
- `GET /runtime/source-gaps/agent-summary` is the compact read-only grouping surface over the same filtered source-gap set; it accepts the same filters as `GET /runtime/source-gaps` excluding append-index cursors, groups by `agent_id` and `source_kind`, returns only `total_count`, `total_groups`, `returned_limit`, and bounded group rows with counts, lifecycle extrema, `output_candidate`, `evidence_role`, and `source_status` buckets, keeps literal seeded or stored agent ids distinct from `agent_id: null` unmapped groups, computes groups before `limit`, and never returns evidence ids, refs, metadata, degraded reason arrays, paths, tmux refs, Hermes payloads, or triggers collection, tmux/filesystem reads, append-only writes, incident/severity/liveness escalation, or control-plane actions
- `GET /runtime/source-gaps/lifecycle` is the compact read-only lifecycle grouping surface over filtered evidence records; it accepts the same filters as `GET /runtime/source-gaps` excluding append-index cursors plus exact additive `lifecycle_state=opened|continuing|resolved|observed_unmapped`, returns only groups that have a source-gap signal, groups by `agent_id`, `source_kind`, and `evidence_role`, reports `total_count` as the matching record aggregate after lifecycle-state filtering, `record_count` as the safe per-group record aggregate, plus `current_status`, `lifecycle_state` (`opened`, `continuing`, `resolved`, or `observed_unmapped`), first/last observed and collected times, `snapshot_count`, and safe `source_status` buckets, filters lifecycle groups before `limit`, returns zero lifecycle groups for unknown lifecycle states, and never returns collector snapshot ids, evidence ids, refs, metadata, degraded reason arrays, paths, tmux refs, Hermes payloads, liveness/productivity/severity inference, or control-plane actions
- `GET /runtime/source-gaps/transition-summary` is the compact read-only count summary over the same lifecycle groups as `GET /runtime/source-gaps/lifecycle`; it accepts the same filters excluding append-index cursors plus exact additive `lifecycle_state=opened|continuing|resolved|observed_unmapped`, returns only `total_records`, `total_groups`, `returned_limit`, `lifecycle_state_buckets`, runtime-only `source_kind_buckets`, `source_status_buckets`, `mapped_state_buckets`, and latest safe observed/collected timestamps, uses bucket `{ group_count, record_count }` values so group and record semantics stay separate, returns zero stable buckets and null timestamps for unknown filters without echoing unsafe input, and never returns group rows, agent ids, evidence ids, refs, collector snapshot ids, metadata, degraded reasons, paths, tmux refs, Hermes payloads, liveness/productivity/severity inference, task semantics, or control-plane actions
- `GET /runtime/source-gaps/trend` is the compact read-only trend surface over the same filtered source-gap set; it accepts the same filters as `GET /runtime/source-gaps` excluding append-index cursors plus `bucket=hour|day`, returns only count buckets by `bucket_start`, computes buckets before `limit`, and never returns agent ids, collector snapshot ids, evidence ids, refs, metadata, degraded reason arrays, paths, tmux refs, Hermes payloads, liveness/productivity/severity inference, or control-plane actions
...
- `GET /storage/replay-manifest` evidence summaries include sparse safe `mapping_decision_buckets` counts and still omit evidence ids, refs, local paths, payloads, metadata, degraded reasons, tokens, webhooks, and control-plane actions
...
- source health can be filtered by exact `collector_snapshot_id`, `agent_id`, `source_kind`, `status`, and post-filter `limit`; accepted agent source-health `source_kind` values are `workspace_root`, `workspace_file`, `workspace_files`, `tmux_observation`, `tmux_session`, `hermes_profile`, and `hermes_session`; task evidence source kinds filter only aggregate `runtime_source_evidence.unmapped_task_evidence` rows
- source health responses include `collected_at`, top-level `collector_snapshot_id`, `actor_id`, summary source/status buckets, bounded agent rows with the same sanitized `collector_snapshot_id`, projected source-health status/time/count fields, `evidence_ref_count`, and `latest_evidence_at`, plus sanitized `runtime_source_evidence.unmapped_tmux_sessions`, `runtime_source_evidence.unmapped_hermes_sources`, and aggregate `runtime_source_evidence.unmapped_task_evidence` rows when present; unmapped tmux/Hermes rows expose only safe `mapping_decision` codes, unmapped task rows expose only `source_kind`, `status`, `observed_count`, `latest_observed_at`, and `mapping_decision_buckets`; the route does not return raw paths, tmux/Hermes/session/profile refs, task refs, evidence refs, metadata, degraded reason arrays, or touch tmux, the filesystem, or collector write paths
...
- collector reports include additive `runtime_source_evidence.unmapped_tmux_sessions`, `unmapped_hermes_sources`, and `unmapped_task_evidence` for observed sources outside the seeded roster
...
- the shell consumes `GET /office/overview`, `GET /office/operations?limit=&state=&agent_id=&severity=`, `GET /agents/:id/workflow?limit=&window=`, `GET /agents/evidence-spine/summary?newest_first=&limit=`, `GET /agents/evidence-spine/source-matrix?newest_first=&limit=`, `GET /incidents?limit=&window=`, `GET /timeline?limit=&window=`, `GET /collectors/controller-snapshot`, `GET /collectors/controller-snapshot/summary`, `GET /collectors/controller-snapshot/evidence-coverage`, `GET /collectors/controller-snapshot/source-health`, `GET /runtime/source-gaps?newest_first=&limit=`, `GET /runtime/source-gaps/summary?newest_first=&limit=`, `GET /evidence-records?agent_id=&newest_first=&limit=`, `GET /evidence-records/ref-rollup?agent_id=&newest_first=&limit=`, `GET /evidence-records/:evidence_id`, `GET /evidence-records/:evidence_id/provenance-bundle`, `GET /evidence-records/:evidence_id/source-context`, `GET /evidence-records/:evidence_id/replay-window`, `GET /accountability/replay`, `GET /accountability/replay/checkpoint-log`, `GET /memory/artifacts`, `GET /peer-watch/alerts`, and `GET /correlations/:correlation_id?limit=&window=`
- `GET /collectors/controller-snapshot/summary` drives the Hub-closed collector snapshot HUD chip with only safe counts, stable source-health bucket totals, timestamp/no-snapshot/error copy, and a click-through to Supervision; the chip must not expose raw collector snapshot items, actor ids, paths, evidence refs, tmux/Hermes/session/profile refs, metadata, degraded reasons, liveness/productivity inference, or control-plane terms
- the React API client also exposes `GET /runtime/source-gaps/agent-summary`, `GET /runtime/source-gaps/trend`, and `GET /runtime/source-gaps/lifecycle`; the operator shell uses the lifecycle helper only for explicit source-gap focus surfaces
...
- HUD evidence coverage focus uses the collector evidence coverage read model and shows the total low-confidence or uncovered employee coverage count, with capped chips and no raw evidence refs or path metadata
...
- the Hub-closed selected-agent inspect peek is the default selection surface: a compact world popover with proof glance, bounded source-health/source-gap/source-matrix peeks, and explicit Now/Evidence/Replay drilldown actions instead of an automatic Hub dump; mapped source-gap world pins can open the same Hub-closed evidence-only inspect peek while keeping unmapped runtime sources passive, and the Evidence action is the explicit boundary before `/evidence-records` reads or deep inspection; the source-gap lifecycle strip keeps mapped and unmapped rows separated, scopes mapped lifecycle reads by selected agent/source/status plus a companion unmapped source read, and uses explicit loading/error/no-snapshot/empty copy without implying health; the source-matrix peek may poll only the exact read-only global matrix route and must render bounded human labels/counts without raw refs, enum keys, paths, tmux/Hermes/session/profile refs, payloads, metadata, degraded reasons, or control-plane terms; the popover, source-health labels, and source-gap peek must not expose local paths, tmux refs, Hermes payloads, raw metadata, raw degraded reasons, or unmapped runtime evidence as selected-agent facts
- the selected-agent Evidence Ledger starts with a compact Proof Compass/basis summary derived from already loaded evidence records plus an explicit selected-agent `GET /evidence-records/ref-rollup?agent_id=<selected>&newest_first=true&limit=12` read only after the Evidence CTA; nonzero Proof Compass bucket chips only jump to the matching already loaded ledger group, ref-rollup Proof Compass rows expose only safe response-local group labels/counts and allowlisted buckets, it redacts local path-like evidence refs in the summary, suppresses local path basenames in inspected source anchors, preserves the detailed ledger behind explicit inspection, keeps the selected evidence source-context bundle behind an explicit Inspect source context disclosure instead of prefetching it, and does not add write/control-plane requests
- the selected-agent Replay Bundle starts with evidence-id scoped replay context when opened from an inspected evidence record, including bounded evidence id/agent/bounds copy, read-only mode, a one-shot selected evidence replay-window card with safe center/bounds/adjacent-row labels, and a Back to Evidence action that returns to the cached selected evidence detail without refetching detail/provenance/checkpoint reads; the compact Replay Proof Ladder follows it and derives from the existing accountability replay audit verdict, showing replayable row counts, collector-only gaps, unsupported gaps, and anchor event counts without exposing raw evidence refs, local paths, or making collector-only gaps look clickable
