...
- `GET /evidence-records?agent_id=&source_kind=&evidence_role=&output_candidate=&evidence_ref=&source_status=&collector_snapshot_id=&correlation_id=&newest_first=&limit=`
...
- evidence-record filters are exact and additive: `agent_id`, `source_kind`, `evidence_role`, `output_candidate`, `evidence_ref`, `source_status`, `collector_snapshot_id`, `correlation_id`, optional `newest_first=true`, and post-filter `limit`; blank string filters are ignored, unknown exact filter values return an empty `items` array, `output_candidate` and `newest_first` accept `true`/`false` via store boolean normalization, invalid or false `newest_first` preserves append order for backward compatibility, and invalid, negative, or missing limits follow the existing read-model limit behavior (`50` default, `200` maximum)
- mapped Hermes profile/session rows use `evidence_role: runtime_presence`; unmapped tmux/Hermes runtime rows remain queryable evidence records with `agent_id: null`, `evidence_role: runtime_unmapped`, and `output_candidate: false`
...
- each collector item exposes additive `source_health` for `workspace_root`, observed workspace files among `inbox.md`/`outbox.md`/`todo.md`, the expected `tmux_session`, and injected Hermes `hermes_profile` / `hermes_session` runtime source facts when that reader is enabled; missing tmux/Hermes sources must be explicit with their expected refs
- the latest collector report exposes additive `runtime_source_evidence.unmapped_tmux_sessions` and `runtime_source_evidence.unmapped_hermes_sources` for observed runtime sources that do not safely match any seeded agent/session
