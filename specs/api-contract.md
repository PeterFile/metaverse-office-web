...
- `GET /accountability/replay/checkpoint-log?limit=&record_kind=` is a sanitized read-only checkpoint log over the replayed append-only record stream; it returns `{ "items": [{ "append_index": number, "record_kind": string, "checkpoint": object|null }] }`, defaults and bounds `limit` like other read routes, optional non-blank `record_kind` exact-matches replayed record kinds before limit, unknown kinds return an empty `items` array, keeps newest selected rows in append order, uses the same bounded checkpoint objects as `checkpoint-summary`, and never returns raw evidence refs, local paths, summaries, metadata, degraded reason arrays, payloads, secrets, tmux/filesystem reads, append-only writes, or control-plane actions.
