...
- evidence record provenance can be inspected through `GET /evidence-records/:evidence_id/provenance-bundle`; it returns bounded record fields plus `source_summary` (`kind`, `status`, `role`, `output_candidate`, `mapped`, and observed/collected timestamps; unknown or unsafe source enum values and invalid/unsafe timestamps are `null` rather than echoed), nullable snapshot/source/replay anchors, and optional sanitized `input_proof` with only `source_format`, `source_index`, `line`, `source_input_ordinal`, and `source_file_ordinal`, but must not expose raw evidence refs, `source_provenance`, snapshot ids or correlation ids inside `source_summary`, metadata, degraded reasons, local paths, tmux/Hermes/session/profile refs, tokens, webhooks, callback endpoints, payload text, or control-plane actions
