...
- evidence refs can be grouped through `GET /evidence-records/ref-rollup` with the same filters; raw `evidence_ref` remains an exact-match input filter, but response groups expose only `evidence_ref: null`, a response-local `evidence_ref_key`, a coarse `evidence_ref_label`, counts, and allowlisted positive source kind/status buckets. The route groups by stored refs internally before `limit` but does not return raw refs, unallowlisted bucket keys, local paths, tmux/Hermes/session/profile refs, URLs, tokens, payloads, metadata, or a reverse lookup from safe keys to raw refs.
