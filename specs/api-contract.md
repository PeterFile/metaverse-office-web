...
- `GET /accountability/replay/checkpoint-log?limit=`
...
- `GET /accountability/replay/checkpoint-log?limit=` is a sanitized read-only checkpoint log over the replayed append-only record stream; it returns `{ "items": [{ "append_index": number, "record_kind": string, "checkpoint": object|null }] }`, defaults and bounds `limit` like other read routes, keeps newest selected rows in append order, uses the same bounded checkpoint objects as `checkpoint-summary`, and never returns raw evidence refs, local paths, summaries, metadata, degraded reason arrays, payloads, secrets, tmux/filesystem reads, append-only writes, or control-plane actions.
...
- diff responses include only snapshot ids/timestamps, top-level summary deltas, source-health bucket deltas, `agent_change_count`, `returned_limit`, and bounded `agent_changes` with `agent_id`, `change_type`, `heartbeat_changed`, and source-health status transitions; they never return raw snapshot items, local paths, tmux refs, evidence refs, metadata, payloads, degraded reasons, task dispatch, claim/complete state, worker orchestration, fabricated liveness/productivity, or severity inference
