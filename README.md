...
- `GET /collectors/controller-snapshot/schema` is a static read-only catalog for the latest, summary, evidence-coverage, source-health, history, and diff routes; it returns only bounded route/filter/enum/limit metadata, exact public `source_kind` filter values/aliases, and static top-level field names, and must not inspect snapshots, collect, read tmux/filesystems/runtime sources, append records, or expose raw ids, refs, paths, degraded reasons, payloads, metadata, or control-plane actions
