...
- `GET /storage/index-health` is a sanitized read-only health view for append-only storage indexes; it returns `{ "item": { "backend": "jsonl"|"sqlite", "status": "ok"|"degraded", "record_count": number, "record_index_count": number|null, "record_evidence_ref_count": number|null, "record_index_drift_count": number|null, "record_evidence_ref_drift_count": number|null, "sidecar_status": "complete"|"stale"|"not_applicable", "record_kind_buckets": object, "latest_record_ts": string|null } }`. JSONL always reports `sidecar_status: "not_applicable"` with null sidecar counts and drift counts. SQLite uses a read-only sidecar count/drift check; missing/unreadable/stale sidecars report `status: "degraded"` and `sidecar_status: "stale"` without falling back to JSONL. The route never returns evidence ids, collector/correlation ids, local paths, raw sqlite paths, stderr, raw evidence refs, payloads, metadata dumps, degraded reasons, tokens, webhooks, tmux/filesystem reads, append-only writes, migrations, backfills, or control-plane actions.
