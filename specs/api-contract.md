...
- `GET /collectors/controller-snapshot/summary` is the safe latest snapshot summary route available for future UI polling; it is read-only over the replayed latest `collector_snapshot` record, never triggers collection, tmux/filesystem/runtime source reads, append-only writes, or control-plane actions, and returns explicit no-snapshot state instead of `null`
